module rptr_empty #(
  ...
  always @(posedge rclk or negedge rrst_n) begin
    if (!rrst_n) begin                 // Reset the FIFO
      rbin <= {ADDR_SIZE+1{1'b0}};
      rptr <= {ADDR_SIZE+1{1'b0}};
    end else begin
      rbin <= rbin_next;              // Shift the binary read pointer
      rptr <= rgray_next;             // Shift the gray read pointer
    end
  end

  assign raddr = rbin[ADDR_SIZE-1:0];                     // Read address calculation from the read pointer
  assign rbin_next = rbin + {{ADDR_SIZE{1'b0}}, (rinc & ~rempty)}; // Increment the read pointer if not empty
